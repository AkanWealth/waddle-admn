...
// //Here is Revenue
// // Add these hooks to your store file after your existing hooks
...
//   const startIndex = (pagination.currentPage - 1) * pagination.itemsPerPage;
//   const paginatedVendors = filteredVendors.slice(startIndex, startIndex + pagination.itemsPerPage);
    ...
    (set) => ({
      ...
      setActiveTab: (tab) =>
        set(
          (state) => ({
            ui: { ...state.ui, activeTab: tab },
            pagination: { ...state.pagination, currentPage: 1 }, // Reset pagination when switching tabs
          }),
          false,
          "setActiveTab"
        ),
...
export const useFilteredVendors = (vendors: IVendor[]) => {
  ...
  return React.useMemo(() => {
    ...
    // Search functionality - search in vendor name and representative
    if (filters.searchTerm) {
      const searchLower = filters.searchTerm.toLowerCase();
      filtered = filtered.filter(vendor => 
        vendor.name.toLowerCase().includes(searchLower) ||
    ...
    // Vendor name filter from main filter
    if (filters.vendor) {
      const vendorLower = filters.vendor.toLowerCase();
      filtered = filtered.filter(vendor => 
        vendor.name.toLowerCase().includes(vendorLower)
    ...
    // Note: Status filter doesn't apply to vendors as they don't have status
    // Date range filter could be implemented if vendors have date fields
