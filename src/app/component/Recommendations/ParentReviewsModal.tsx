...
const ParentReviewsModal: React.FC = () => {
  ...
  const [totalRecommendation, setTotalRecommendation] = useState<number>(0);
  const [percentageCount, setPercentageCount] = useState<number>(0);
  ...
  const fetchReviews = async (pageNum = 1) => {
    try {
      ...
      if (context == "place") {
        ...
        if (response.success) {
          const { reviews, totalPages, total } = response.data;
          setReviews(reviews);
          setTotalPages(totalPages || 1);
          setPage(pageNum);
          setPercentageCount(total);
        } else {
      ...
      if (context == "event") {
        const response =
          await recommendationService.getAllRecommendationsEventByPage(
            itemId,
            pageNum
          );
        console.log("This is the response for our event stuff");
        if (response.success) {
          const { comments, totalPages, total } = response.data;
          setReviews(comments);
          setTotalPages(totalPages || 1);
          setPage(pageNum);
          setPercentageCount(total);
  ...
  const fetchPercentage = async (context: "place" | "event") => {
    try {
      if (context == "event") {
        const response =
          await recommendationService.getRecommendationEventPercentage(itemId);
        setTotalRecommendation(response.data.percentage);
        console.log(response, "This is the percentage");
      }
      if (context == "place") {
        const response =
          await recommendationService.getRecommendationPlacePercentage(itemId);
        setTotalRecommendation(response.data.percentage);
        console.log(response, "This is the percentage for places");
      }
    } catch (error) {
      console.log("Failed to fetch percentage", error);
    }
  };
  const handleUnflagReview = async (commentId: string) => {
    try {
      if (context == "event") {
        const response = await recommendationService.flagOrUnflagEventComment(
          commentId,
          "APPROPRIATE"
        );
        if (response.success) {
          ...
          showMessage(
            "Review Unflagged",
            "Review has been added to public view",
            "success"
          );
          setReviews((prevReviews) =>
            prevReviews.map((review) =>
              review.id === commentId
                ? { ...review, status: "APPROPRIATE" }
                : review
            )
          );
        }
      ...
      if (context == "place") {
        const response = await recommendationService.flagOrUnflagPlaceComment(
          commentId,
          "APPROPRIATE"
        );
        if (response.success) {
      ...
      console.log(error);
      showMessage("Failed", "Failed to unflag this review", "success");
    }
  };
  const handleFlagReview = async (commentId: string) => {
    try {
      if (context == "event") {
        const response = await recommendationService.flagOrUnflagEventComment(
          commentId,
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <p className="text-sm text-[#565C69] font-normal">
                Based on {percentageCount} Waddle reviews
              </p>
            </div>
          </div>
        </div>
              ...
              <div key={review.id} className="border rounded-lg p-4 shadow-sm">
                ...
                {review.status == "INAPPROPRIATE" ? (
                  <button
                    ...
                    className="mt-3 flex items-center gap-1 text-black  bg-[#E5E7EF] rounded-md px-3 py-1 text-sm hover:bg-red-50 font-medium"
                  >
                    <IoFlag className="w-4 h-4 text-red-600" />
                    Unflag
                  </button>
                ) : (
                  <button
                    onClick={() => handleFlagReview(review.id)}
                    className="mt-3 flex items-center gap-1 text-red-600  bg-[#E5E7EF] rounded-md px-3 py-1 text-sm hover:bg-red-50 font-medium"
                  >
