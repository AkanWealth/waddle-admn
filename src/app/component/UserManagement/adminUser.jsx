...
export default function AdminUsersTable({ currentPage, searchTerm, statusFilter, dateFilter, mobileView }) {
        ...
        try {
            ...
            if (response && response.admin) {
                ...
                const transformedAdmins = response.admin.map(admin => {
                    let status = "Pending";
                    if (admin.email_verify && admin.isActivated) {
                        status = "Active";
                    } else if (!admin.email_verify) {
                        status = "Pending";
                    } else if (admin.email_verify && !admin.isActivated) {
                        status = "Inactive";
                    }
                    return {
                        id: admin.id,
                        ...
                        registrationDate: new Date(admin.createdAt).toISOString().split('T')[0],
                        status,
    ...
    const handleResendInvite = async (adminId) => {
        try {
            await authService.makeAuthenticatedRequest(`/api/v1/host/invite/${adminId}`, {
                method: 'POST'
            });
    ...
    const handleDeactivateAdmin = async (adminId) => {
        try {
            await authService.makeAuthenticatedRequest(`/api/v1/host/activate/${adminId}`, {
                method: 'DELETE'
            });
    ...
    const handleReactivateAdmin = async (adminId) => {
        try {
            await authService.makeAuthenticatedRequest(`/api/v1/host/web/${adminId}/reactivate`, {
                method: 'PATCH'
            });
    ...
    const handleDeleteAdmin = async (adminId) => {
        try {
            ...
            await authService.makeAuthenticatedRequest(`/api/v1/host/web/${adminId}`, {
                method: 'DELETE'
            });
